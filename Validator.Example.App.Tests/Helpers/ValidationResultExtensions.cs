...
namespace Validator.Example.App.Tests.Helpers {
    public static class ValidationResultExtensions {
        ...
        public static void ShouldPass(this ValidationResult source, Guid guid) {
            ...
            if (fail != null)
                Assert.Fail($"Validation {guid} failed when it should have passed: [{(fail.Keys.Any() ? fail.Keys.Aggregate((a, b) => $"{a}/{b}") : "N/A")}] - {fail.Message}");
        ...
        public static void ShouldFail(this ValidationResult source, Guid guid) {
            ...
            var pass = source.Passes.FirstOrDefault(x => x.Guid == guid);
            if (pass != null)
                Assert.Fail($"Validation {guid} passed when it should have failed: [{pass.Keys.Aggregate((a, b) => $"{a}/{b}")}] - {pass.Message}");
